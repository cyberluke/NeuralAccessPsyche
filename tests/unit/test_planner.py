...
@pytest.fixture
def enable_planner(monkeypatch):
    """Temporarily enable the planner for testing."""
    monkeypatch.setattr("core.persona.planner.PLANNER_ENABLED", True)
...
class TestPlanner:
    @pytest.mark.asyncio
    async def test_valid_plan_parsed(self, enable_planner):
    ...
    @pytest.mark.asyncio
    async def test_planner_temperature_in_range(self, enable_planner):
        engine = FakeEngine(content=json.dumps(VALID_PLAN))
        await build_rhetorical_plan(engine, "x", temperature=0.9)
        assert 0.3 <= engine.last_request.temperature <= 0.6

    @pytest.mark.asyncio
    async def test_planner_uses_structured_output(self, enable_planner):
        engine = FakeEngine(content=json.dumps(VALID_PLAN))
        await build_rhetorical_plan(engine, "x")
        assert engine.last_request.response_format["type"] == "json_schema"

    @pytest.mark.asyncio
    async def test_planner_failure_returns_none(self, enable_planner):
        engine = FakeEngine(raise_error=True)
        plan = await build_rhetorical_plan(engine, "x")
        assert plan is None

    @pytest.mark.asyncio
    async def test_invalid_json_returns_none(self, enable_planner):
        engine = FakeEngine(content="not json at all")
        plan = await build_rhetorical_plan(engine, "x")
        assert plan is None

    @pytest.mark.asyncio
    async def test_planner_disabled_returns_none(self):
        """When PLANNER_ENABLED is False, should return None immediately."""
        engine = FakeEngine(content=json.dumps(VALID_PLAN))
        plan = await build_rhetorical_plan(engine, "x")
        assert plan is None
