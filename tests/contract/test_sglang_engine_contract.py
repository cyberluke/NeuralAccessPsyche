...
class TestAliasRouting:
    def test_both_aliases_map_to_same_upstream(self):
        assert MODEL_ALIAS_MAP["nram-deepseek-r1-qwen-7b"] == "nram-deepseek-r1-qwen-7b"
        assert MODEL_ALIAS_MAP["deepseek-r1-qwen-7b-baseline"] == "nram-deepseek-r1-qwen-7b"

    def test_only_nram_alias_is_steering_enabled(self):
        assert "nram-deepseek-r1-qwen-7b" in NRAM_ENABLED_ALIASES
        assert "deepseek-r1-qwen-7b-baseline" not in NRAM_ENABLED_ALIASES

    def test_is_nram_enabled_true_for_nram_alias(self, engine):
        assert engine._is_nram_enabled(make_request("nram-deepseek-r1-qwen-7b"))

    def test_is_nram_enabled_false_for_baseline(self, engine):
        assert not engine._is_nram_enabled(make_request("deepseek-r1-qwen-7b-baseline"))

    def test_nram_disabled_via_option(self, engine):
        req = make_request("nram-deepseek-r1-qwen-7b", nram={"enabled": False})
...
class TestPayloadConstruction:
    def test_baseline_has_no_processor(self, engine):
        req = make_request("deepseek-r1-qwen-7b-baseline")
    ...
    def test_nram_payload_has_processor_and_params(self, engine):
        req = make_request("nram-deepseek-r1-qwen-7b")
    ...
    def test_model_alias_resolved_in_payload(self, engine):
        req = make_request("deepseek-r1-qwen-7b-baseline")
        payload = engine._build_upstream_payload(req, nram_enabled=False)
        assert payload["model"] == "nram-deepseek-r1-qwen-7b"
